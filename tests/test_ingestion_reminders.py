...
def _bind_all_cursor_credentials(session, reminder_env):
    ...
    mock_cursor_key_exchange(cred_service.cursor_client, email="cursor-bound@example.com")
...
def test_build_scheduler_includes_cursor_sync_tick(reminder_env):
    scheduler = build_scheduler(reminder_env["config"], MagicMock(), MagicMock(), MagicMock())
    job_ids = {job.id for job in scheduler.get_jobs()}
    assert "cursor_sync_tick" in job_ids
    assert "daily_cursor_sync" not in job_ids


def test_run_cursor_sync_tick_delegates(reminder_env):
    service = ReminderService(reminder_env["config"], reminder_env["sf"], MagicMock(), MagicMock())
    with patch("pulse.channels.reminders.scheduler.run_sync_tick", return_value=2) as mock_tick:
        synced = service.run_cursor_sync_tick()
    assert synced == 2
