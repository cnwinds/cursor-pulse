...
from pulse.chat.admin_tools import AdminContext, DEFAULT_ROUTER
...
def test_list_pending_reviews_tool(owner_member):
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    from sqlalchemy.orm import sessionmaker
    ...
    team, repo = make_team_repo(session)
    ctx = AdminContext(
        config=AppConfig(tenant=TenantConfig(slug="test")),
        session=session,
        team_id=team.id,
        repo=repo,
        member=owner_member,
    ...
    result = DEFAULT_ROUTER.execute(ctx, "list_pending_reviews", {})
