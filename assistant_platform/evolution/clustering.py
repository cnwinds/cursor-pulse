...
def _suggest_diff_text(tag: str, session_ids: list[str]) -> str:
    ...
    return f"# Suggested prompt diff for failure tag: {tag}\n# Sessions: {len(session_ids)}\n{hint}"
...
def cluster_low_score_reviews(db_session: Session) -> list[FailureClusterRow]:
    ...
    """
    reviews = list(db_session.scalars(select(SessionReviewRow).where(SessionReviewRow.score < _LOW_SCORE_THRESHOLD)))
    ...
    for tag, session_ids_set in tag_sessions.items():
        ...
        cluster = db_session.scalar(select(FailureClusterRow).where(FailureClusterRow.tag == tag))
        if cluster is None:
