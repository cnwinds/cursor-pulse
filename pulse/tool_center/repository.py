...
class ToolCenterRepository:
    ...
    def account_has_associated_data(self, account_id: str) -> bool:
        checks = (
            select(AiAccountCredential.id).where(AiAccountCredential.account_id == account_id).limit(1),
            select(UsageSummary.id).where(UsageSummary.account_id == account_id).limit(1),
            select(UsageIngestion.id).where(UsageIngestion.account_id == account_id).limit(1),
            select(AccountQuotaSnapshot.id).where(AccountQuotaSnapshot.account_id == account_id).limit(1),
            select(KeyLoan.id).where(KeyLoan.source_account_id == account_id).limit(1),
            select(UsageDailyAggregate.id).where(UsageDailyAggregate.account_id == account_id).limit(1),
        )
        for query in checks:
    ...
    def delete_account(self, account_id: str) -> str:
        """Return ``hard`` or ``soft`` depending on whether the row was removed."""
        account = self.get_account(account_id)
        if not account:
            raise ValueError("账号不存在")
