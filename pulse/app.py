...
from pulse.bot.dingtalk.client import start_dingtalk_bot
from pulse.bot.reminders.scheduler import build_scheduler
...
def run_app(config: AppConfig) -> None:
    if not config.admin.dingtalk_user_ids:
        logger.warning(
            "admin.dingtalk_user_ids 未配置：所有用户将拥有管理员权限。"
            "生产环境请在 config.yaml 或 DINGTALK_ADMIN_USER_IDS 中设置。"
        )
