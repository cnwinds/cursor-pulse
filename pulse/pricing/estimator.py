    ...
    *,
    ...
    if rec.cost_raw in (CostRaw.INCLUDED, CostRaw.FREE):
        if scope == "auto_composer":
            normalized = normalize_cursor_model_name(rec.model)
            model_name = rec.model if normalized.startswith("composer") else "auto"
...
def _round_pool_bucket(bucket: dict) -> dict:
    ...
    tokens = {
        k: int(bucket["tokens_by_model"].get(k, 0) or 0)
        for k in breakdown
        if int(bucket["tokens_by_model"].get(k, 0) or 0) > 0
    }
    return {
