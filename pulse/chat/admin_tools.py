    ...
    def _register_defaults(self) -> None:
        ...
        self.register(
            AdminTool(
                ...
                description="列出待管理员审核的截图/低置信度提交",
                handler=_list_pending_reviews,
            )
        )
        self.register(
            AdminTool(
...
def _run_evolution(ctx: AdminContext, args: dict) -> ToolResult:
    ...
    if ctx.session_factory is None:
        return ToolResult(
            tool="run_evolution",
            status="skipped",
            message="自进化需要 session_factory。",
            capability="evolution:run",
        )
    result = run_memory_evolution(ctx.session_factory, ctx.config)
    return ToolResult(
        tool="run_evolution",
        status="executed",
        message=f"自进化完成：新增 {result['principles']} 条原则，执行 {result['actions']} 个动作。",
        capability="evolution:run",
        detail=result,
    )
...
def _list_pending_reviews(ctx: AdminContext, args: dict) -> ToolResult:
    from pulse.periods import current_period
