...
def format_pool_spend_note(
    *,
    ...
    if cursor_pools:
        ...
        third_party_pool = cursor_pools.get("third_party") or {}
        auto_spend = float(auto_pool.get("spend_usd") or 0)
        api_spend = float(api_pool.get("spend_usd") or 0)
        third_party_spend = float(third_party_pool.get("spend_usd") or 0)
        if auto_spend <= 0 and api_spend <= 0 and third_party_spend <= 0:
            return ""
        ratio_line = ""
        ...
        third_party_line = (
            f"\n· 三方模型：${third_party_spend:.2f}" if third_party_spend > 0 else ""
        )
        return (
