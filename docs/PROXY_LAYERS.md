...
| 变量 | 层 | 含义 |
|------|----|------|
| 代理地址（系统设置） | A | 写入一键命令，用户设为 `HTTPS_PROXY` |
...
| 助手回复「暂时不可用」，日志 `httpx.ProxyError: 403` | 壳层把代理地址（`:8317`）当成了系统 `HTTPS_PROXY`，LLM 打到 MITM | 不要 `export HTTPS_PROXY=<代理地址>`；或在项目 `.env` 写 `HTTPS_PROXY=` / `HTTP_PROXY=` 清空；`cursor-pulse.sh` 也会自动剥离 MITM 地址 |
