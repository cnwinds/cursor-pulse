...
| 组件 | 进程/命令 | 数据 |
|------|-----------|------|
| 机器人 + 调度 | `pulse channel` | 长连接，不可多实例抢同一机器人 |
...
| 群绑定 | `team_settings.dingtalk`（pulse.db） | 自动捕获 openConversationId |
...
5. **openConversationId**：
   ...
   - 或留空，在群内 **@机器人** 一次，自动写入 `team_settings.dingtalk`
...
在 `docker/` 目录执行（见 `docker/README.md`）：
...
服务：`web` / `assistant` / `channel`（不是根目录旧版单服务 `pulse`）。

持久化目录：

- `docker/data` → 数据库 + 原始文件 + 群绑定
- `docker/config.yaml` → 只读挂载

### 2.4 Docker + Postgres

```bash
# 在 docker/ 目录；修改 docker-compose.postgres.yml 中 POSTGRES_PASSWORD，并同步 DATABASE_URL
cd docker
...
容器内需安装 Postgres 驱动：镜像构建时已包含 `psycopg` 时可直接用；否则在 `docker/Dockerfile` 中 `pip install -e ".[postgres]"`。
...
pulse init-db
pulse channel                     # 前台或 systemd/supervisor 守护
# 可选另一终端：
pulse web --port 8080
...
- 示例：「催一下没交的」「重新聚合 2026-06」「导出本月用量」

| 能力码 | 对话/工具示例 |
|--------|----------------|
| `tasks:nudge` | 催未提交成员 |
| `metrics:aggregate` | 重新聚合 |
| `reports:publish` | 发群月报（需 `pulse channel` 侧钉钉连接） |
| `submissions:read` | 查看摄取记录（Web 摄取记录页） |
| `evolution:run` | 记忆自进化 |
...
2. 检查管理后台 **设置 → 钉钉** 或 `team_settings.dingtalk` 是否已更新。
...
1. 文本 `上报 智谱 85`、截图识别成功、Web 手工录入 → **直接 `confirmed` 入库**，计入统计。
2. 截图识别置信度不足 → 提示改用手工文本上报，**不**进入待审队列。
3. Web **摄取记录**（`/ingestions`）可查看主指标与提交渠道。
4. 历史遗留 `pending_review` 记录（若有）仍可在摄取记录页通过/拒绝，新提交不再产生待审。
...
| 现象 | 可能原因 | 处理 |
|------|----------|------|
| 机器人无回复 | Stream 断连、凭证错误 | 重启 `pulse channel`；检查 AppKey/Secret |
...
| 小脉催办无效果 | Web 未连钉钉 | `publish_report`/`nudge` 需 messenger；生产建议 `pulse channel` 同机或配置钉钉凭证 |
...
| `pulse.db`（含 `team_settings.dingtalk`） | 随配置变更 |
...
| Postgres | `docker/docker-compose.postgres.yml` + `DATABASE_URL` |
...
```ini
[Unit]
Description=Cursor Pulse Channel
...
ExecStart=/opt/cursor-pulse/.venv/bin/pulse channel -c /opt/cursor-pulse/config.yaml
Restart=always
RestartSec=10
...
| `pulse channel` | Channel Adapter + 调度器 |
