...
| `pulse/bot/reminders/scheduler.py` | 每日 02:00 同步任务 |
...
| `pulse/bot/dingtalk/handler.py` | Key 绑定 + Cursor CSV 拦截 |
...
| `pulse/bot/dingtalk/handler.py` | 同上 |
| `pulse/bot/commands.py` | `confirm_submission` → `confirm_ingestion` |
...
| `pulse/bot/pending_submission.py` | 重命名为 `pending_credential_bind.py`（Cursor）+ `pending_ingestion.py`（手工） |
...
- Modify: `pulse/bot/reminders/scheduler.py`
...
**Files:**
- Modify: `pulse/bot/commands.py`
- Modify: `pulse/bot/dingtalk/handler.py`
...
- [ ] 删除 `pulse/bot/pending_submission.py` 中 Cursor 账号选择流程（如已被 credential 绑定替代）
